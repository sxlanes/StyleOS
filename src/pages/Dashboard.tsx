...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white flex font-sans overflow-hidden">
            {/* Background Image for Integration */}
            <div className="absolute inset-0 z-0 opacity-20 pointer-events-none">
                <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#D4AF37]/10 blur-[100px] rounded-full"></div>
            </div>

            {/* Sidebar - Integrated & Translucent */}
            <aside className="w-64 bg-black/40 backdrop-blur-2xl border-r border-white/5 hidden md:flex flex-col p-4 sticky top-0 h-screen z-20">
                <div className="mb-8 flex items-center gap-3 px-2">
                    <div className="w-6 h-6 bg-[#D4AF37] rounded shadow-[0_0_15px_rgba(212,175,55,0.3)]"></div>
                    <span className="font-bold text-lg tracking-tight font-serif text-white/90">StyleOS</span>
                </div>

                <nav className="flex-1 space-y-1">
                    <div
                        onClick={() => setView('dashboard')}
                        className={`p-2.5 rounded-lg flex items-center gap-3 text-sm font-medium cursor-pointer transition-all ${view === 'dashboard' ? 'bg-white/5 text-[#D4AF37] border border-white/5 shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <LayoutDashboard className="w-4 h-4" /> Dashboard
                    </div>
                    <div
                        onClick={() => setView('calls')}
                        className={`p-2.5 rounded-lg flex items-center gap-3 text-sm font-medium cursor-pointer transition-all ${view === 'calls' ? 'bg-white/5 text-[#D4AF37] border border-white/5' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <Phone className="w-4 h-4" /> Journal d'Appels
                    </div>
                    <div
                        onClick={() => setView('calendar')}
                        className={`p-2.5 rounded-lg flex items-center gap-3 text-sm font-medium cursor-pointer transition-all ${view === 'calendar' ? 'bg-white/5 text-[#D4AF37] border border-white/5' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                    >
                    ...
                    {isAdmin && (
                        <>
                            <div
                                ...
                                className={`p-2.5 rounded-lg flex items-center gap-3 text-sm font-medium cursor-pointer transition-all ${view === 'accounting' ? 'bg-white/5 text-[#D4AF37] border border-white/5' : 'text-blue-400/70 hover:text-blue-300 hover:bg-blue-500/10'}`}
                            >
                                <FileText className="w-4 h-4" /> Comptabilité (IA)
                            </div>
                            <div
                                onClick={() => setView('clients')}
                                className={`p-2.5 rounded-lg flex items-center gap-3 text-sm font-medium cursor-pointer transition-all ${view === 'clients' ? 'bg-white/5 text-[#D4AF37] border border-white/5' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                            >
                ...
                <div className="pt-4 border-t border-white/5">
                    <div className="flex items-center gap-3 mb-3 px-2">
                        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 border border-white/10 flex items-center justify-center text-[10px] font-bold">
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <p className="text-[10px] text-green-500/80 flex items-center gap-1"><span className="w-1 h-1 rounded-full bg-green-500"></span> En ligne</p>
                        </div>
                    </div>
                    <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 text-red-400/80 hover:bg-red-500/10 hover:text-red-300 p-2 rounded-lg transition-colors text-xs font-bold uppercase tracking-wider">
                        <LogOut className="w-3 h-3" /> Déconnexion
                    </button>
                </div>
            </aside>

            {/* Main Content - Compact & No Scroll if possible */}
            <main className="flex-1 p-6 h-screen overflow-hidden flex flex-col relative z-10">
                <header className="flex justify-between items-center mb-6 shrink-0">
                    <div>
                        <h1 className="text-xl font-bold text-white tracking-tight">Vue d'ensemble</h1>
                        <p className="text-xs text-gray-500 font-medium">Bienvenue, {user?.user_metadata?.business_name}.</p>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={addMockAppointment} className="bg-white/5 border border-white/10 text-white px-3 py-1.5 rounded-lg font-bold text-xs hover:bg-white/10 transition-colors flex items-center gap-2">
                            <Plus className="w-3 h-3" /> Simuler RDV
                        </button>
                        {isAdmin && (
                            <button onClick={() => setView('campaigns')} className="bg-[#D4AF37] text-black px-4 py-1.5 rounded-lg font-bold text-xs hover:bg-[#c5a02e] transition-all shadow-lg shadow-[#D4AF37]/20">
                                Nouvelle Campagne
                            </button>
                        )}
                    </div>
                    ...
                    {view === 'dashboard' && (
                        <>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                                {/* Calls Card */}
                                <div className="bg-white/[0.03] border border-white/5 p-4 rounded-2xl hover:bg-white/[0.05] transition-colors relative overflow-hidden group">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="p-2 bg-blue-500/10 text-blue-400 rounded-lg">
                                            <Phone className="w-4 h-4" />
                                        </div>
                                        <span className="text-[10px] font-bold bg-green-500/10 text-green-400 px-2 py-0.5 rounded-full">+12%</span>
                                    </div>
                                    <div className="text-2xl font-bold mb-0.5 text-white tracking-tight">{stats.calls}</div>
                                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Appels IA</div>
                                </div>

                                {/* Revenue Card */}
                                <div className="bg-white/[0.03] border border-white/5 p-4 rounded-2xl hover:bg-white/[0.05] transition-colors">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="p-2 bg-[#D4AF37]/10 text-[#D4AF37] rounded-lg">
                                            <TrendingUp className="w-4 h-4" />
                                        </div>
                                    </div>
                                    <div className="text-2xl font-bold mb-0.5 text-white tracking-tight">{stats.revenue} €</div>
                                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Chiffre d'Affaires</div>
                                </div>
                                ...
                                <div className="bg-white/[0.03] border border-white/5 p-4 rounded-2xl hover:bg-white/[0.05] transition-colors">
                                    ...
                                    <div className="text-2xl font-bold mb-0.5 text-white tracking-tight">- {stats.expenses} €</div>
                                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Dépenses</div>
                                </div>

                                {/* Net Profit Card */}
                                <div className="bg-gradient-to-br from-green-900/40 to-black border border-green-500/20 p-4 rounded-2xl relative overflow-hidden group">
                                    <div className="absolute top-0 right-0 w-24 h-24 bg-green-500/10 blur-[40px] rounded-full group-hover:bg-green-500/20 transition-all"></div>
                                    <div className="flex justify-between items-start mb-3 relative z-10">
                                        <div className="p-2 bg-green-500/10 text-green-400 rounded-lg border border-green-500/10">
                                            <Wallet className="w-4 h-4" />
                                        </div>
                                    </div>
                                    <div className="text-2xl font-bold mb-0.5 text-white relative z-10 tracking-tight">{stats.net} €</div>
                                    <div className="text-[10px] text-green-400/80 relative z-10 font-bold uppercase tracking-wider">Bénéfice Net</div>
                                </div>
                            </div>

                            {/* Compact Accounting Actions */}
                            <div className="grid md:grid-cols-2 gap-4 mb-6">
                                {/* Hidden File Input */}
                                <input type="file" ref={fileInputRef} onChange={handleFileUpload} hidden accept="image/*,application/pdf" />

                                <div className="bg-blue-900/10 border border-blue-500/20 p-4 rounded-2xl flex items-center justify-between group hover:border-blue-500/40 transition-all">
                                    <div className="relative z-10">
                                        <h3 className="font-bold text-sm text-blue-100 flex items-center gap-2 mb-1">
                                            <FileText className="w-4 h-4" /> Export Comptable
                                        </h3>
                                        <p className="text-[10px] text-blue-300/70">Rapport PDF/Excel pour expert-comptable.</p>
                                    </div>
                                    <button onClick={() => alert("PDF Généré !")} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-xs shadow-lg shadow-blue-500/20">
                                        Générer
                                    </button>
                                </div>

                                <div onClick={handleScanClick} className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between group hover:border-[#D4AF37]/50 transition-all cursor-pointer relative overflow-hidden">
                                    ...
                                    <div className="relative z-10">
                                        <h3 className="font-bold text-sm text-white flex items-center gap-2 mb-1">
                                            {isScanning ? <Loader2 className="w-4 h-4 animate-spin text-[#D4AF37]" /> : <Camera className="w-4 h-4 text-[#D4AF37]" />}
                                            {isScanning ? "Analyse..." : "Scan Facture IA"}
                                        </h3>
                                        <p className="text-[10px] text-gray-400">Scan intelligent de factures.</p>
                                    </div>
                                    <button disabled={isScanning} className="relative z-10 bg-[#D4AF37] text-black px-4 py-2 rounded-lg font-bold text-xs hover:bg-white transition-all shadow-lg hover:shadow-[#D4AF37]/20 disabled:opacity-50">
                                        Scanner
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md flex-1">
                                <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                                    <h3 className="font-bold text-sm text-white">Activité Récente</h3>
                                    <button className="text-[10px] font-bold uppercase tracking-wider text-[#D4AF37] hover:text-white">Tout voir</button>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-xs text-left">
                                        <thead className="bg-black/40 text-gray-500 uppercase font-bold tracking-wider">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-white/5 text-gray-300">
                                            {appointments.map((appt, i) => (
                                                <tr key={i} className="hover:bg-white/5 transition-colors group">
                                                    <td className="px-6 py-3 font-bold text-white">{appt.client_name}</td>
                                                    <td className="px-6 py-3 text-gray-400">{(appt as any).type || "Coupe"}</td>
                                                    <td className="px-6 py-3">
                                                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase border ${appt.status === 'Confirmé' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'}`}>
                                                            {appt.status}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-3 text-gray-500 font-mono">{(appt as any).time}</td>
                                                    <td className="px-6 py-3 text-white font-bold font-mono text-right">{appt.amount} €</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
