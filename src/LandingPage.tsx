    ...
    <div className="w-full md:w-3/4 mx-auto h-full bg-black flex flex-col rounded-[2rem] border border-white/10 overflow-hidden relative shadow-2xl">
        ...
        <div className="flex-1 p-4 space-y-4 font-sans text-[10px] bg-black relative">
            <div className="flex gap-2 relative z-10">
                <div className="p-3 bg-white/10 rounded-2xl rounded-tl-none text-gray-300 max-w-[85%] border border-white/5">Est-ce que vous faites aussi la taille de barbe à l'ancienne ?</div>
            </div>
            <div className="flex gap-2 justify-end relative z-10">
                <div className="p-3 bg-primary/10 text-white rounded-2xl rounded-tr-none max-w-[85%] border border-primary/20">Oui absolument ! Notre forfait "Rituel Barbier" à 35€ inclut la taille, le soin à la serviette chaude et l'application d'huile. Je vous réserve un créneau ?</div>
            </div>
        </div>
    </div>
);
...
const GuaranteeMock = () => (
    <div className="w-full h-full bg-black rounded-xl border border-white/10 flex flex-col items-center justify-center relative overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] group">
        ...
        <div className="mt-8 px-6 py-2 bg-primary/10 rounded-full text-primary text-xs font-black uppercase tracking-[0.2em] border border-primary/20 relative z-10">Satisfait ou Remboursé</div>
    </div>
)
...
const ProcessAuditMock = () => (
    <div className="w-full h-full bg-black border border-white/10 rounded-xl p-8 relative overflow-hidden flex flex-col justify-center shadow-xl group">
        ...
        <div className="space-y-4 relative z-10">
            {['Analyse SEO Local', 'Vitesse Site Actuel', 'Tunnel de Réservation'].map((item, i) => (
...
const FeatureRow = ({ title, description, icon: Icon, align = 'left', action, visual }: any) => {
    // Reveal TITLE first, then VISUAL+CONTENT delayed significantly to avoid overlap
    return (
...
function LandingPage() {
    ...
    // Step 3: 0.8-0.9 (In) [GAP: 0.75-0.8]
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-primary selection:text-black font-sans">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <div className="mt-48">
                        <FeatureRow title="Web Elite" description='Design "Dark Luxury" sur-mesure. SEO Local dominant. Votre salon n’est plus une adresse, c’est une destination.' icon={Monitor} align="left" visual={<WebEliteMock />}
                            action={<a href="#demo" className="flex items-center gap-3 text-primary text-xs font-bold uppercase tracking-widest hover:text-white transition-colors group cursor-pointer inline-block mt-4">Voir les réalisations <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></a>}
                        />
                    </div>
            ...
            <div className="relative z-10 bg-black border-t border-white/5 py-24"><ScrollReveal><div className="max-w-7xl mx-auto px-6"><FeatureRow title="Garantie Élite" description="Risque Zéro. Si StyleOS ne finance pas son propre coût dès le premier mois grâce aux économies de commissions et au gain de productivité, nous vous remboursons intégralement l'abonnement." icon={ShieldCheck} align="right" visual={<GuaranteeMock />} /></div></ScrollReveal></div>
            ...
            <div id="pricing" className="relative z-10 bg-black pb-32 border-t border-white/5 pt-24"><ScrollReveal><PricingSection /></ScrollReveal></div>
            ...
            <footer className="bg-black py-12 border-t border-white/5 text-center md:text-left">
                <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-8">
                    ...
                    <div><ul className="space-y-2 text-xs text-gray-500"><li><button onClick={() => scrollToSection('features')} className="hover:text-white transition-colors">Fonctionnalités</button></li><li><button onClick={() => scrollToSection('pricing')} className="hover:text-white transition-colors">Tarifs</button></li></ul></div>
            ...
            {isDemoOpen && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-500"><div className="absolute inset-0 bg-black/95 backdrop-blur-2xl" onClick={() => setIsDemoOpen(false)}></div><div className="relative w-full max-w-6xl aspect-video bg-black rounded-[2.5rem] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(212,175,55,0.2)] animate-in zoom-in-95 duration-500"><button onClick={() => setIsDemoOpen(false)} className="absolute top-6 right-6 z-20 text-white"><CloseIcon /></button></div></div>)}
        </div>
    );
}
